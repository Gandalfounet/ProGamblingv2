...
Trade.prototype.startCheckingDeposit = function(offerid, steamIDUser, total_offer){
    var that = this
    this.intervalDeposit[offerid] = setInterval(function(){
        that.checkDepositOffer(offerid, steamIDUser, total_offer)
    }, 10000)
}
Trade.prototype.checkDepositOffer = function(offerid, steamIDUser, total_offer){
    var that = this;
    this.instances['bot_3'].manager.getOffer(offerid, function(err, offer){
        if(offer.state == 3){
            clearInterval(that.intervalDeposit[offerid])
            offer.getReceivedItems((err, items) => {
                User.updateUserCoin(steamIDUser, total_offer*100, (err, res) => {
                                        that.io.emit('updateAllCoins', {steamid: steamIDUser, amount : (total_offer*100)})
                                        that.io.emit('notifyAllSuccess', {steamid: steamIDUser, error: "You have been credited of " + total_offer*100 + " credits."})       
                                    }) 
                
            })
            
        }else if(offer.state != 1 && offer.state != 2){
            //Clear the interval here
            clearInterval(that.intervalDeposit[offerid])
            that.intervalDeposit[offerid] = false
        }
    })
}
